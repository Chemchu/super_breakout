use avian2d::{
    ...
    dynamics::rigid_body::{GravityScale, Restitution, RigidBody},
};
use bevy::{
    asset::Assets,
    ecs::{
...
use crate::{
    ball::{BallPool, BallShot, setup_ball},
    constants::{BALL_COLOR, BALL_RADIUS, BALL_SHAPE, PADDLE_HEIGHT, PADDLE_WIDTH, PADDLE_Y_POS},
    wall::CollisionLayer,
};
...
pub fn on_ball_and_paddle_collision(event: On<CollisionStart>, paddle_query: Query<&Paddle>) {
    println!("{:#?}", event);
    let paddle_entity = event.collider1;
    let ball_entity = event.collider2;

    if paddle_query.contains(paddle_entity) {
        println!("{:#?}", paddle_entity);
    }
}
